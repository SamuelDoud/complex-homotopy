...
class Line(object):
    ...
    def create_points(self):
        z = np.linspace(self.start,self.end,self.number_of_points) #this is the first value on the domain
        points = []
        for step_index in range(self.number_of_points): #there is always at least a start and an end, start is already included, so end must be forced in
            f_z = self.function.evaluateAt(z[step_index]) #evaluate the function at this complex number
